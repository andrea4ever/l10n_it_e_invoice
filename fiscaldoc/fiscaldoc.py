...
class FiscalDocHeader(orm.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        if not vals:
            return super(FiscalDocHeader, self).create(cr, uid, vals, context)
        causale_id = vals.get('tipo_doc', False)
        part_id = vals.get('partner_id', False)
        if not causale_id or not part_id:
            return super(FiscalDocHeader, self).create(cr, uid, vals, context)
        caus_obj = self.pool.get('fiscaldoc.causalidoc')
        part_obj = self.pool.get('res.partner')
        if caus_obj.browse(cr, uid, causale_id, context).causale_pa:
            partner = part_obj.browse(cr, uid, part_id, context)
            if not partner.codice_ipa:
                raise osv.except_osv(
                    _('Errore'),
                    _('Hai utilizzato una causale PA ma' \
                      ' il partner non ha il codice IPA'))
        return super(FiscalDocHeader, self).create(cr, uid, vals, context)
        ...
        for tags in parser.getElementsByTagName("esito"):
            ...
            for node in tags.getElementsByTagName("nomeFileSdi"):
                for value in node.childNodes:
                    ...
                    vals.update({
                        'note': note})
                    self.pool.get('fiscaldoc.header').write(
                        cr, uid, [invoice_id],
...
class einvoice_history(orm.Model):
    ...
    _columns = {
        'name': fields.many2one(
            'fiscaldoc.header', 'Invoice', required=True, ondelete='cascade'),
